...
def prepare_dataset():
    ...
    for file in os.listdir(path):
        ...
        classes = line.split(',')
        label = numpy.ndarray((1, output_classes))

        for c in range(1, len(classes)):
            label[(0,classes[c])] = 1.0

        image = matplotlib.image.imread(file_path)
        image.reshape((1, input_size_height * input_size_width))
        pixel_type = numpy.dtype(numpy.uint8)

        image = (image - (numpy.iinfo(pixel_type).max - numpy.iinfo(pixel_type).min) / 2.0) / (
        numpy.iinfo(pixel_type).max - numpy.iinfo(pixel_type).min)
...
W_fc_1st = weight_variable([(input_size_height / 2) * (input_size_width / 2) * 64, 64])
b_fc_1st = bias_variable([64])
...
W_fc_2nd = weight_variable([64, output_classes])
...
for i in range(learning_steps):
    ...
    if i % 64 == 0:
        ...
        #batch = batch.reshape((1, batch_size * input_size_height * input_size_width))
        #label = label.reshape((batch_size*output_classes))

        train_accuracy = accuracy.eval(feed_dict={x_place: batch, y_place: label})
        print("Step %d: training accuracy = %g" % (i, train_accuracy))

    train_step.run(feed_dict={x_place: batch, y_place: label})
